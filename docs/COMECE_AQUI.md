...
## 🚀 Setup Inicial (TODOS devem fazer primeiro)

**ANTES DE COMEÇAR**: Leia [`SETUP_COMPLETO.md`](SETUP_COMPLETO.md) para configurar o ambiente completo (30 min)

Este guia te ensina a:
- Instalar dependências (Node.js, PostgreSQL, etc)
- Configurar backend e banco de dados
- Configurar frontend
- Entender autenticação profissional com HttpOnly Cookies
...
**PASSO 3**: Configure o ambiente seguindo [`SETUP_COMPLETO.md`](SETUP_COMPLETO.md)
...
### 🔐 Autenticação Profissional (IMPORTANTE!)
...
**Como funciona:**
1. **Login**: Backend retorna token em HttpOnly Cookie (não acessível via JavaScript)
2. **Requisições**: Cookie é enviado automaticamente pelo navegador
3. **Logout**: Backend limpa o cookie

**Durante desenvolvimento com MOCK:**
- Temporariamente use `sessionStorage` para simular autenticação
- Quando backend estiver pronto, migre para HttpOnly Cookies

Leia mais em [`SETUP_COMPLETO.md`](SETUP_COMPLETO.md#-autenticação-profissional)
...
- [`SETUP_COMPLETO.md`](SETUP_COMPLETO.md) - **Setup completo do ambiente** (LEIA PRIMEIRO!)
...
**TODOS**: Leia [`SETUP_COMPLETO.md`](SETUP_COMPLETO.md) primeiro para configurar o ambiente
...
**Autenticação**: Use HttpOnly Cookies (seguro) ao invés de localStorage (vulnerável)
